...
# Environment variables that control make.sh:
#
...
# (Or just pass them to the make.sh command line.)
#
# GOBUILDTIMELOGFILE: If set, make.sh and all.bash write
...
if [ ! -f run.bash ]; then
	echo 'make.sh must be run from $GOROOT/src' 1>&2
	exit 1
fi

if [ "$GOBUILDTIMELOGFILE" != "" ]; then
	echo $(LC_TIME=C date) start make.sh >"$GOBUILDTIMELOGFILE"
fi

# Test for Windows.
case "$(uname)" in
*MINGW* | *WIN32* | *CYGWIN*)
	echo 'ERROR: Do not use make.sh to build on Windows.'
...
# Run dist bootstrap to complete make.sh.
...
# The bootstrap+rm above are the final step of make.sh.
# If something must be added, add it to cmd/dist's cmdbootstrap,
# to avoid needing three copies in three different shell languages
# (make.sh, make.bat, make.rc).
